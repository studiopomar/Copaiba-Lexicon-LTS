...
class FullAudioLoaderWorker(QThread):
    """
    Worker que carrega TUDO do áudio:
    - Data bruta (para espectrograma)
    - Data reduzida (para waveform)
    - Metadados (framerate, duration)
    """
    finished = Signal(dict)
    error = Signal(str)

    def __init__(self, path: Path, max_points: int = 8000, normalize: bool = True):
        super().__init__()
        self.path = path
        self.max_points = max_points
    ...
    def run(self):
        try:
            # 1. Carrega data bruta
            data, framerate = read_wav_file(str(self.path))
            if data.size == 0:
                self.error.emit(f"Falha ao ler arquivo: {self.path.name}")
                return

            duration = data.size / float(framerate)

            # 2. Gera waveform reduzida (times, values)
            # Reutiliza a lógica de load_waveform_sync mas com os dados já em memória
            total_samples = data.size
            plot_data = data.copy()
            
            if self.normalize:
                max_amp = np.max(np.abs(plot_data))
                if max_amp > 0:
                    plot_data /= max_amp

            if total_samples > self.max_points:
                step = max(1, int(total_samples // self.max_points))
                n_usable = (total_samples // step) * step
                truncated = plot_data[:n_usable]
                blocks = truncated.reshape(-1, step)
                mins = blocks.min(axis=1)
                maxs = blocks.max(axis=1)

                v_res = np.empty(mins.size * 2, dtype=np.float32)
                v_res[0::2] = mins
                v_res[1::2] = maxs

                t_steps = (np.arange(len(blocks)) * step) / float(framerate)
                t_res = np.empty(v_res.size, dtype=np.float32)
                t_res[0::2] = t_steps
                t_res[1::2] = t_steps
            else:
                v_res = plot_data
                t_res = np.arange(total_samples, dtype=np.float32) / float(framerate)
