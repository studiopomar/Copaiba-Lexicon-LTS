...
class MenuBuilder:
    ...
    def create_menus(self):
        ...
        # Dock toggles (permite reabrir docks fechados)
        mw.m_view.addSeparator()
        mw.m_view_docks = mw.m_view.addMenu("Painéis")
        if hasattr(mw, '_dock_params'):
            mw.m_view_docks.addAction(mw._dock_params.toggleViewAction())
        if hasattr(mw, '_dock_wave'):
            mw.m_view_docks.addAction(mw._dock_wave.toggleViewAction())
        if hasattr(mw, '_dock_presets'):
            mw.m_view_docks.addAction(mw._dock_presets.toggleViewAction())
        if hasattr(mw, '_dock_spectrogram_config'):
            mw.m_view_docks.addAction(mw._dock_spectrogram_config.toggleViewAction())
