...
logger = logging.getLogger("copaiba.spectrogram")
...
class SpectrogramWidget(QWidget):
    ...
    def set_audio_data(self, wave_data: np.ndarray, sample_rate: int, duration: float, wav_path: str = None):
        ...
        """
        logger.debug(f"set_audio_data: shape={wave_data.shape if wave_data is not None else None}, sr={sample_rate}, dur={duration:.3f}s")
        
        # Verifica cache por caminho de arquivo
        if wav_path and wav_path == self._current_wav_path and self._cache_valid:
            logger.debug(f"Cache válido para {wav_path}, reutilizando")
    ...
    def _on_spectrogram_ready(self, spectrogram_data):
        logger.debug(f"Espectrograma pronto: shape={spectrogram_data.shape}")
    ...
    def _on_spectrogram_error(self, error_msg):
        logger.error(f"Erro no espectrograma: {error_msg}")
        self._spectrogram_cache = None
        self._cache_valid = False
        self._worker = None
