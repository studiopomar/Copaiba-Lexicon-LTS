...
class CheatsheetDialog(QDialog):
    ...
    def __init__(self, parent: 'MainWindow' = None):
        ...
        self.setWindowTitle("Atalhos e Comandos - O Pomar")
        self.resize(800, 600)
        self._setup_ui()
    ...
    def _setup_ui(self):
        ...
        self._add_section("📂 Arquivo e Projetos", [
            ("Abrir voicebank...", "Ctrl + O", "Apre a pasta de um voicebank e carrega o oto.ini"),
        ...
        self._add_section("⏯️ Reprodução", [
        ...
        self._add_section("🖥️ Visualização", [
    ...
    def _add_section(self, title: str, items: list[tuple[str, str, str]]):
        ...
        section_frame.setStyleSheet("QFrame#SectionFrame { background-color: rgba(255, 255, 255, 5); border-radius: 8px; }")
        ...
        header.setStyleSheet("color: #4CAF50; margin-top: 10px; margin-bottom: 5px;")
        ...
        for row, (action, shortcut, desc) in enumerate(items):
            action_lbl = QLabel(f"<b>{action}</b>")
            shortcut_lbl = QLabel(f"<code style='color: #FFC107;'>{shortcut}</code>")
            desc_lbl = QLabel(f"<span style='color: #888;'>{desc}</span>")
            
            grid.addWidget(action_lbl, row, 0)
            grid.addWidget(shortcut_lbl, row, 1)
