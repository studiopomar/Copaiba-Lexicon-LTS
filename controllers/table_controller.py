...
class TableController:
    ...
    def __init__(self, main_window: 'MainWindow'):
        ...
        # Timer para debounce do filtro (evita lag ao digitar rápido)
        self._filter_timer = QTimer()
        self._filter_timer.setSingleShot(True)
        self._filter_timer.setInterval(200)  # 200ms de debounce
    ...
    def delete_alias(self) -> None:
        ...
        alias_item = mw.table.item(row, mw.COL_ALIAS)
        alias_name = alias_item.text() if alias_item else f"linha {row + 1}"

        reply = QMessageBox.question(
            mw, "Confirmar Exclusão",
            f"Deseja realmente excluir o alias \'{alias_name}\' (linha {row + 1})?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No
        )
    ...
    def filter_table(self, text: str) -> None:
        """Filtra a tabela com debounce (evita lag ao digitar rápido)."""
        self._pending_filter_text = text
        self._filter_timer.start()  # Reinicia o timer a cada tecla

    def _do_filter(self) -> None:
        """Executa o filtro de fato após o debounce."""
        text = self._pending_filter_text
        mw = self.mw
        text_lower = text.strip().lower()
        for row in range(mw.table.rowCount()):
            if not text_lower:
                mw.table.setRowHidden(row, False)
                continue
            alias_item = mw.table.item(row, mw.COL_ALIAS)
            filename_item = mw.table.item(row, mw.COL_FILENAME)

            alias_text = alias_item.text().lower() if alias_item else ""
            filename_text = filename_item.text().lower() if filename_item else ""

            should_show = text_lower in alias_text or text_lower in filename_text
            mw.table.setRowHidden(row, not should_show)
>>>>>>> Stashed changes
